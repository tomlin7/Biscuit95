...
class ExtensionViewer(Toplevel):
    ...
    def __init__(self, master, *args, **kwargs) -> None:
        ...
        Label(container, text="…" * 100).pack(side=tk.TOP, fill=tk.X, pady=5)

        self.author = WebLinkLabel(container, text=f"Tommy")
        self.author.config(font=self.base.settings.uifont)
        self.description = Label(container, text=f"Hello, world!")
        ...
        container.pack(side=tk.TOP, fill=tk.X, expand=False, pady=(0, 5), padx=(0, 5))

        # ------------------------------------------------------------------
        # README viewer (markdown -> html)
        # ------------------------------------------------------------------
        self.body = HtmlFrame(
            container, messages_enabled=False, vertical_scrollbar=False
        )
    ...
    def show(self, ext: ExtensionGUI) -> None:
        ...
        try:
            ...
            if owner and repo:
                self.author.set_link(f"https://github.com/{owner}/{repo}")
                self.link.set_link(f"https://github.com/{owner}/{repo}")

...
class Welcome(BaseEditor):
    ...
    def __init__(self, master, exists=False, editable=False, *args, **kwargs) -> None:
        ...
        self.left = Frame(self.container)  # , padx=10)
    ...
    def create_quick_group(self):
        ...
        QuickItem(
            quick, "History", Icons.HISTORY, self.toggle_history, ["Ctrl", "R"]
        ).pack(fill=tk.X, expand=True)
    ...
    def create_recent_group(self):
        frame = Frame(self.right)
        frame.pack(fill=tk.X, pady=10)

        Label(frame, text="HISTORY", font=("Fira Code", 20, "bold")).pack(
            anchor=tk.W, side=tk.LEFT
        )

        BorderedIconButton(frame, icon=Icons.CLOSE, event=self.toggle_history).pack(
            anchor=tk.E, side=tk.RIGHT, padx=5, pady=5
        )
        ...
        recents = Frame(self.right, width=300)
        ...
        for i, p in enumerate(self.base.history.folder_history.list):
            RecentItem(recents, *p).grid(row=i, column=0, sticky=tk.EW, pady=1)

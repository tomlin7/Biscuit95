...
from biscuit.common.ui import ButtonsEntry, Frame, IconButton, Label
...
class SearchEditor(BaseEditor):
    ...
    def __init__(self, master, exists=False, editable=False, *args, **kwargs) -> None:
        ...
        # Header area
        self.header_container = Frame(self.container)
        self.header_container.pack(fill=tk.X, anchor=tk.N, pady=(10, 0), padx=5)

        # Main Search Row
        self.search_row = Frame(self.header_container)
        self.search_row.pack(fill=tk.X)

        # Results area
        self.results = SearchResults(self.container, self)

        # Collapse button
        self.collapse_btn = IconButton(
            self.search_row,
            Icons.FOLD,
            self.results_toggle_collapse,
            icon2=Icons.UNFOLD,
        )
        ...
        # Extra Buttons
        self.extra_buttons = Frame(self.search_row)
        self.extra_buttons.pack(side=tk.LEFT, padx=(5, 0))

        IconButton(self.extra_buttons, Icons.FILTER, self.toggle_filter).pack(
            side=tk.LEFT
        )
        IconButton(self.extra_buttons, Icons.REPLACE, self.toggle_replace).pack(
            side=tk.LEFT
        )

        self.extra_buttons_2 = Frame(self.search_row)
        self.extra_buttons_2.pack(side=tk.LEFT, padx=(5, 0))

        IconButton(self.extra_buttons_2, Icons.REFRESH, self.results.search).pack(
            side=tk.LEFT
        )
        IconButton(self.extra_buttons_2, Icons.CHEVRON_LEFT, lambda: None).pack(
            side=tk.LEFT
        )
        IconButton(self.extra_buttons_2, Icons.CHEVRON_RIGHT, lambda: None).pack(
            side=tk.LEFT
        )

        self.count_label = Label(self.extra_buttons_2, text="0/0", font=("Segoe UI", 9))
        self.count_label.pack(side=tk.LEFT, padx=5)

        # Replace Row
        self.replace_row = Frame(self.header_container)
        self.replace_row.pack(fill=tk.X, padx=(30, 0))

        # Normal Entry for Replace
        self.replace_entry_container = Frame(self.replace_row)
        ...
        self.replace_btns = Frame(self.replace_row)
        self.replace_btns.pack(side=tk.LEFT, padx=(5, 0), pady=2)

        IconButton(self.replace_btns, Icons.REPLACE, self.results.replace_single).pack(
            side=tk.LEFT
        )
        IconButton(self.replace_btns, Icons.REPLACE_ALL, self.results.replace_all).pack(
            side=tk.LEFT
        )

        # Filter Row
        self.filter_row = Frame(self.header_container)
        # self.filter_row.pack(fill=tk.X, padx=(30, 0)) # Initially hidden

        # Include Box
        self.include_container = Frame(self.filter_row)
        self.include_container.pack(
            side=tk.LEFT, fill=tk.X, expand=True, pady=2, padx=(0, 2)
        )
        # self.include_container.config(padx=1, pady=1)

        Label(self.include_container, text="Include:", font=("Segoe UI", 8)).pack(
            side=tk.LEFT, padx=(0, 1), fill=tk.BOTH
        )
        ...
        # Exclude Box
        self.exclude_container = Frame(self.filter_row)
        self.exclude_container.pack(
            side=tk.LEFT, fill=tk.X, expand=True, pady=2, padx=(2, 0)
        )
        # self.exclude_container.config(padx=1, pady=1)

        Label(self.exclude_container, text="Exclude:", font=("Segoe UI", 8)).pack(
            side=tk.LEFT, padx=(0, 1), fill=tk.BOTH
        )
        ...
        self.filter_extras = Frame(self.filter_row)
        self.filter_extras.pack(side=tk.LEFT, padx=(5, 0))
        self.files_btn = IconButton(
            self.filter_extras, Icons.FILES, self.toggle_open_editors_only
        )
        self.files_btn.pack(side=tk.LEFT)

        # Empty State Overlay
        self.empty_state = Frame(self.container)
        self.empty_state.place(relx=0.5, rely=0.55, anchor=tk.CENTER)

        Label(
            self.empty_state, text="Search All Files", font=("Segoe UI", 12, "bold")
        ).pack(pady=5)
        Label(
            self.empty_state,
            text="Hit enter to search. For more options:",
            font=("Segoe UI", 10),
        ).pack(pady=(0, 20))

        self.shortcuts_container = Frame(self.empty_state)
    ...
    def add_shortcut_label(self, icon, text, shortcut):
        row = Frame(self.shortcuts_container)
        row.pack(fill=tk.X, pady=2)

        Label(row, text=icon, font=("codicon", 11)).pack(side=tk.LEFT, padx=(0, 5))
        Label(row, text=text, anchor=tk.W, font=("Segoe UI", 10)).pack(side=tk.LEFT)
        Label(row, text=shortcut, anchor=tk.E, font=("Segoe UI", 10)).pack(
            side=tk.RIGHT, padx=(20, 0)
        )
    ...
    def results_toggle_collapse(self, *_) -> None:
        ...
        state = self.results.treeview.item(
            self.results.treeview.get_children()[0], "open"
        )
